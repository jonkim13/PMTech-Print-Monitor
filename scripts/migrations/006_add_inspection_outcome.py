"""Add inspection_outcome column to jobs (Phase D).

Phase D adds the inspector pass/fail gate. Internal and External
jobs route through an inspection step before being treated as
completed; this migration adds the single column that backs the
gate's state. Design jobs continue to skip inspection.
...
Existing rows pick up ``inspection_outcome='pending'`` via SQLite's
column DEFAULT applied at ALTER ADD time — no per-row UPDATE is
needed.
...
Values: ``'pending'`` (the default, no recordable outcome yet),
``'pass'`` (inspector cleared the job), ``'fail'`` (inspector
rejected the job). The deriver in
``app/domains/work_orders/status_sync.py`` translates these into the
existing five-state job status enum — no new status values are
introduced on ``jobs``.

Two layers of idempotence
-------------------------
1. ``MigrationRunner.is_applied('006_add_inspection_outcome')``
   short-circuits the whole script before any DDL.
2. The single ALTER is guarded by ``add_column_if_missing`` (PRAGMA
   table_info check). If the registry row was lost but the column
   already exists, the apply path is a no-op for the schema and only
   records the registry row.

Usage
-----
    # Read-only preview (default if neither flag is passed):
    python scripts/migrations/006_add_inspection_outcome.py
    python scripts/migrations/006_add_inspection_outcome.py --dry-run

    # Apply. Creates a backup of work_orders.db first.
    sudo systemctl stop print-farm-monitor
    python scripts/migrations/006_add_inspection_outcome.py --apply
...
DESCRIPTION = (
    "Add inspection_outcome column to jobs (Phase D inspection gate)"
)
...
# Mirror this list byte-identical in
# app/domains/work_orders/job_repository.py's _init_tables block so
# fresh DBs and migrated DBs converge.
...
def print_triage(db_path: str, missing: List[str], present: List[str],
    ...
    if not missing:
        ...
        for col in missing:
            print("  + jobs.{:<22} {}".format(col, column_def_for(col)))
    if present:
        print()
    ...
    print()
    print("Existing jobs would receive inspection_outcome='pending' via "
          "SQLite DEFAULT.")
...
def print_apply_summary(db_path: str, backup_path: Optional[str],
    ...
    print()
    print("Migration 006 — Inspection outcome column")
    print("  DB: {}".format(db_path))
    if backup_path:
    ...
    print("  Existing jobs receive inspection_outcome='pending' via "
          "SQLite DEFAULT")
