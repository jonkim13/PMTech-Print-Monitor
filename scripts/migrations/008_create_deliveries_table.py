"""Create the deliveries table (Phase F).

Phase F adds the terminal "Deliver Part to Customer" step. An
operator records delivery against a genuinely ``completed`` work
order and the WO moves to the manual terminal status ``delivered``.
This migration adds the single table that records each delivery; it
lives in the existing ``data/work_orders.db`` (same file as the WO it
references, so the FK is real and there is no cross-DB concern).

The migration is **create-only**:

- 1 new table (``deliveries``) + its index.
- No data writes, no ALTER, no DROP.
- The ``work_orders`` table is NOT touched. ``delivered`` is a status
  *value* written into the existing free-text ``status`` column — no
  schema change, no CHECK constraint to widen.

The table DDL is imported verbatim from
``app/domains/work_orders/repository.py`` (DELIVERIES_SCHEMA_STATEMENTS)
so a migrated DB and a fresh ``_init_tables`` install converge
byte-for-byte — there is only one copy.

Two layers of idempotence
-------------------------
1. ``MigrationRunner.is_applied('008_create_deliveries_table')``
...
-----
    ...
    python scripts/migrations/008_create_deliveries_table.py
    python scripts/migrations/008_create_deliveries_table.py --dry-run

    # Apply. Backs up work_orders.db first (the file always exists).
    sudo systemctl stop print-farm-monitor
    python scripts/migrations/008_create_deliveries_table.py --apply
...
from app.domains.work_orders.repository import (  # noqa: E402
...
DESCRIPTION = (
    "Create deliveries table in work_orders.db (Phase F delivery step)"
)
...
def classify_tables(conn: sqlite3.Connection) -> Tuple[List[str], List[str]]:
    """Split DELIVERIES_TABLES into (missing, present) by current schema."""
    missing: List[str] = []
    present: List[str] = []
    for table in DELIVERIES_TABLES:
...
def print_triage(db_path: str, file_existed: bool,
    ...
    if not missing:
        print("  (none — the deliveries table already exists)")
    else:
...
def print_apply_summary(db_path: str, backup_path: Optional[str],
    ...
    print()
    print("Migration 008 — Deliveries table")
    print("  DB: {}".format(db_path))
    if backup_path:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if file_existed:
        ...
        missing, present = list(DELIVERIES_TABLES), []
